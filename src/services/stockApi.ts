...
// S&P 500 Stock tickers for error fallback display
// Auto-generated from https://github.com/datasets/s-and-p-500-companies
// Total: 503 stocks
const STOCK_TICKERS = [
  { symbol: 'MMM', name: '3M' },
...
export async function fetchStockAssets(limit: number = 503): Promise<Asset[]> {
  try {
    const url = `${API_BASE_URL}/api/stocks?limit=${limit}`;
    ...
    const tickersToShow = STOCK_TICKERS.slice(0, limit);
    return tickersToShow.map((stock, index) => ({
      id: `stock-${stock.symbol.toLowerCase().replace('-', '')}`,
      rank: index + 1,
      name: stock.name,
      symbol: stock.symbol,
      marketCap: 0,
      ...
      change24h: 0,
      type: 'stock' as const,
      error: `Unable to load ${stock.name} data`,
    }));
  }
}
